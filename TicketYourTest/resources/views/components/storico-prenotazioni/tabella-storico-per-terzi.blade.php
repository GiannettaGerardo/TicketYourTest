    ...
    <table class="table table-striped">
        ...
        <thead>
            <tr>
                ...
                @if (Session::get('Attore') == 2){{-- l'utente loggato è un datore di lavoro --}}
                    <td>risultato tampone</td>
                @else
        ...
        <tbody>
            ...
            @foreach ($prenotazioni as $prenotazione)
                ...
                <tr>
                    ...
                    @if (Session::get('Attore') == 2){{-- l'utente loggato è un datore di lavoro --}}
                        <td>{{ $prenotazione->esito_tampone }}</td>
                    @else
                        <td><a class="btn btn-primary"
                                href="{{ route('referto.tampone', $prenotazione->id_referto) }}">scarica</a></td>
                    @endif

                    @if (Session::get('Attore') == 3){{-- l'utente loggato è un medico --}}

                        @if ($prenotazione->risultato_comunicato == 0)
                            <td>
                                <form action="{{ route('storico.prenotazioni.medico') }}" method="POST"
                                    style="justify-content: space-around; align-items: center" class="rowP">
